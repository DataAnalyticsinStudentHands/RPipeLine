TCEQ.daily_aggregate <- function(dataframe, aggregate_method="max", minimum_number_of_measurements=18) {
  aggregate_by <- list(day=cut(dataframe$hourofday, breaks="day"), site=dataframe$site, region=dataframe$region, param_id=dataframe$param_id)
  ...
  aggregated_dataframe$hour <- dataframe$hourofday[match(aggregated_dataframe$x, dataframe$value)]
  aggregated_dataframe$value <- help.TCEQ.normalize_numeric_values(aggregated_dataframe$x)
  
  aggregated_dataframe[which(aggregated_dataframe$number_of_measurements <= minimum_number_of_measurements)] <- NA
...
TCEQ.hourly_aggregate <- function(dataframe, aggregate_method="mean", minimum_number_of_measurements=8) {
  ...
  aggregated_dataframe$hourofday <- help.TCEQ.normalize_datetime_values(aggregated_dataframe$hourofday)
  aggregated_dataframe$value <- help.TCEQ.normalize_numeric_values(aggregated_dataframe$x)
...
TCEQ.rolling_average <- function(value_vector, rolling_length=8, maximum_allowed_missing=2) {
  ...
  for (value_index in 1:length(value_vector)) {
    ...
    for (rolling_offset in 0:(rolling_length-1)) {
      if (is.na(values[value_index + rolling_offset]))
        number_missing <- number_missing + 1
      else {
        value_total <- value_total + values[value_index + rolling_offset]
      }
    }
...
help.TCEQ.normalize_source_data <- function(dataframe, omit_NA_rows=TRUE, omit_flagged_rows=TRUE) {
  ...
  if (omit_flagged_rows)
    data <- subset(dataframe, dataframe$flag=="")
  return(dataframe)
}
  ...
  if (is.na(AQS_codes)) {
    if (is.na(region)) {
      if("AQS_codes" %in% colnames(dataframe))
        AQS_codes <- unique(dataframe$AQS_codes)
      else
...
#      if("AQS_codes" %in% colnames(dataframe)){
#        matching_rows <- subset(dataframe, dataframe$AQS_codes == AQS_codes[AQS_code_index])
#      }
#     else {
#        matching_rows <- subset(dataframe, dataframe$site == AQS_codes[AQS_code_index])
#      }
      matching_rows <- subset(data,dataframe$param_id == pollutants[pollutant_index])
      if (nrow(matching_rows) > 0) {
        ...
        if (is.element('eighthrrolling', stats))
          eight_hour_rolling_averages <- TCEQ.rolling_average(hourly_averages$value)
        if (is.element('eighthrmax', stats)) {
          before_4pm <- subset(eight_hour_rolling_averages, as.numeric(format(eight_hour_rolling_averages$hourofday, format="%H")) <= 16)
          eight_hour_maximums <- TCEQ.daily_aggregate(before_4pm$value)
        }
      }
    }
  }
  return(list(hourly_averages, hourly_maximum, eight_hour_rolling_averages, eight_hour_maximums))
}
