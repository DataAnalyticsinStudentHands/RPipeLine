process.using_QuARM <- function(dataframe, event_column='event', seeds=1:3, training_sample_sizes=c(1, .4, .2), alpha=1.598, support=0.005, remove_insignificant_results=TRUE, remove_redundant_results=TRUE) {
	source('process_libs/QuARM.R')
	results <- QuARM.perform_associative_rule_mining(dataframe, event_column, seeds, training_sample_sizes, alpha, support)
...
process.using_GenARM <- function(dataframe, number_of_generations=200, gene_pool_size=30, initial_attribute_probability=0.25, seed=NULL,
  ...
  source('process_libs/GenARM.R')
  rules <- GenARM.perform_genetic_algorithm_rule_mining(dataframe, number_of_generations, gene_pool_size, initial_attribute_probability, seed,
  ...
  statistics <- NULL
  if (with_statistics)
    statistics <- GenARM.statistics_report(dataframe, rules, gene_pool_size, weights)
