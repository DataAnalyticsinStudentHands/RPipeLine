...
confidence.int <- function(testbin) {
  ...
  perc_exp <- (x11 + x01)/(x11 + x01 + x10 + x00)
  C <- perc_exp*(x11+x10) # expected cases if independent
  K <- perc_exp*(x01+x00) # expected controls if independent
  chi_squared <- (x11-C)^2/C + (x01-(x11+x10-C))^2/(x11+x10-C) + (x10-K)^2/K + (x00-(x01+x00-K))^2/(x01+x00-K)
