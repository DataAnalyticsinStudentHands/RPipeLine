...
GA.modules <- function(data, generations = 200, upop_size = 30, Pin = 0.25, seed = NULL,
                        weights = rep(1,4), pc = 0.6, pc2 = 0.5, k = 4, pm_range = c(0.001, 0.01),
                        pt_range = c(0.9, 0.6), fitness = c("sum", "arithmetic", "geometric")) {
  ...
  source('~/Desktop/my papers/7 - GA for ARM/R files/GAfinal/GAfunctions.R')
  
  # Chose between aritmetic or geometric average for fitness function
  fitness <- match.arg(fitness)
  if (fitness == "sum")
    source('~/Desktop/my papers/7 - GA for ARM/R files/GAfinal/fitness_sum.R')
  else if (fitness == "arithmetic")
    source('~/Desktop/my papers/7 - GA for ARM/R files/GAfinal/fitness_arit.R')
  else 
    source('~/Desktop/my papers/7 - GA for ARM/R files/GAfinal/fitness_geom.R')
...
#   maxdist <- sapply(as.data.frame(rbind(Ldist, Udist)), max)
  ...
  Upop <- fitness.fun(Upop, upop_size, genes_num, test, weights)
...
# Adjust fitness to avoid rule repetitions and redundancy
  ...
  for (i in 1:upop_size) {
    if(!is.na(rep.penalty(Upop,i,upop_size)))
      Upop$fitness[i] <- Upop$fitness[i] - weights[3]*rep.penalty(Upop,i,upop_size)
    if(!is.na(redundancy(Upop,i,upop_size)))
      Upop$fitness[i] <- Upop$fitness[i] - weights[4]*redundancy(Upop,i,upop_size)
  }
  ...
  for (n in 1:generations) {
    ...
    Upop2 <- fitness.fun(Upop2, upop_size, genes_num, test, weights)
    ...
    for (i in 1:(upop_size*2)) {
      if(!is.na(rep.penalty(temp,i,upop_size*2)))
        temp$fitness[i] <- temp$fitness[i] - weights[3]*rep.penalty(temp,i,upop_size*2)
      if(!is.na(redundancy(temp,i,upop_size*2)))
        temp$fitness[i] <- temp$fitness[i] - weights[4]*redundancy(temp,i,upop_size*2)
    }
...
#### OUTPUT REPORT ####
GAreport <- function(test, Upop, upop_size = 30, weights = rep(1,5), filename = "GAreport.csv") {
  ...
  for (i in 1:upop_size) {
    ...
    support[i] <- sum(rowSums(testbin)==length(A)+1)/nrow(testbin) # support is no o.f., but it's useful
    length[i] <- length(A)/genes_num
  ...
  Output <- cbind(Rule = (1:upop_size), Output, length = length, OR = OR, repetition = penalty, redundancy = red, supp = support, stats)
