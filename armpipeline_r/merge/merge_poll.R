...
# labels: poll_code, name. A name should be available for every pollutant in E.
  ...
  allcodes <- (unique(E$poll_code))
	...
	for (code in allcodes) {
		...
		newcolumns <- c(h0, v0, h1, v1, h2, v2)
		Out[, newcolumns] <- NA
	}
...
# 	}
  ...
  for (i in 1:nrow(Out)) {
    for (j in 1:length(allcodes)) {
      ...
      if(!is.na(exprow0)) {
      Out[i,j*6] <- E$hour[exprow0]
      Out[i,j*6+1] <- E$value[exprow0]
      }
      if(!is.na(exprow1)) {
      Out[i,j*6+2] <- E$hour[exprow1]
      Out[i,j*6+3] <- E$value[exprow1]
      }
      if(!is.na(exprow2)) {
      Out[i,j*6+4] <- E$hour[exprow2]
      Out[i,j*6+5] <- E$value[exprow2]
      }
    }
  }
