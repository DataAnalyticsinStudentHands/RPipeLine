...
#' @usage customCI(x, transactions, transNot, reuse = TRUE, alpha = 1.96)
...
#' @param transNot The negated transaction data set for calculations.
#' @param reuse A logical indicating if information in quality slot should be reuse for calculating the measures.
#' @param alpha The critical value from normal distribution
#' @return output$lowCI: the lower bound of a confidence interval
#' @return output$highCI: the higher bound of a confidence interval.
#' @export
customCI <- function(x, transactions, transNot, reuse = TRUE, alpha = 1.96) {
