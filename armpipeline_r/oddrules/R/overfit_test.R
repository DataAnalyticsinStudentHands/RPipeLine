...
#' @usage overfit_test(whole_training, testing, seed=1:3, trainingsize=0.5*nrow(whole_training), alpha = 1.598, support = 0.005)
#' @param whole_training The training data set
#' @param testing The testing data set
#' @param seed A vector of values for randomly splitting the data set
#' @param trainingsize A vector of sizes for training sets to mine from
#' @param alpha A threshold for significance
#' @param support The minimum support for rule mining
#' @return list: 3 matrices with the number of rules mined for each iteration of size and seed meeting minimum support, significance, and are nonredundant, and a dataframe of said successfull rules
#' @export
overfit_test <- function (whole_training, testing, seed=1:3, trainingsize=0.5*nrow(whole_training), alpha = 1.598, support = 0.005) {
  ...
  for (i in 1:length(seed)) {
    for (j in 1:length(trainingsize)){
      ...
      if (sum(training$event) < 1){
        ...
        tr=arules:::match(rhs(rules),tr)
        tr=replace(tr,tr==1,TRUE)
        tr=replace(tr,tr==NA,FALSE)
        tr=as.logical(tr)
        rulesCases <- subset(rules, subset = tr)
        ...
        data2rules=as(rulesCases,"data.frame")
        sig=(is.finite(data2rules$oddsRatio) & data2rules$significance==TRUE)
        significant=subset(rulesCases,subset=sig)
        ...
        if (nrow(signRules)<1) {
          ...
          quality(significant) <- cbind(quality(significant), improvement_customCI_par(significant))
          
          data3rules=as(significant,"data.frame")
          significant <- subset(significant, subset = data3rules$impCIcustom_par)
          ...
          if (C[j,i]>0){
            ...
            quality(ruleTest) <- cbind(quality(ruleTest), improvement_customCI_par(ruleTest))
